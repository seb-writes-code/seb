...
server.tool(
  ...
  async (args) => {
    ...
    // Signal to the host that send_message was called during this turn.
    // The host uses this flag to suppress the final agent output (which would
    // otherwise duplicate the message already delivered via IPC).
    const flagDir = path.join(IPC_DIR, 'flags');
    fs.mkdirSync(flagDir, { recursive: true });
    fs.writeFileSync(path.join(flagDir, 'send_message_called'), '1');
